...
Pipeline: 
1. Load EMG data from CSV (multiple channels)
2. Calculate MAV for each channel and average across channels
3. Use a state machine to classify activations as dots/dashes based on duration
4. Detect letter and word boundaries based on rest durations

Translate dots and dashes into letters and words based on timing gaps. 

Input: CSV file with EMG data (1000Hz sampling rate)
Output 1: Sequence of more code dots (.) and dashes (-)
Output 2: Translated Alphanumeric characters from Morse code
"""
...
def calculate_mav(channel_data):
    ...
    for ch_name, signal in channel_data.items():
        ...
        while start + WINDOW_SIZE <= len(signal):
            ...
            mav = np.mean(np.abs(window))  # Calculate MAV for each window position
    ...
    # Average across channels (row-wise mean) to get a single MAV value per window
    averaged_mav = np.mean(all_mavs, axis=0)  # Row based mean, [num_windows,]
    
    return averaged_mav # 1D array of MAV values averaged across channels, one per window position
...
class MorseDetector:
    ...
    def __init__(self):
        ...
        self.saved_gap_count = 0   # saved gap count for noise recovery
    ...
    def process_mav(self, mav_value):
        ...
        if self.state == "IDLE":
            ...
            if is_active:
                ...
                # Activation ended, classify as dot, dash, or noise
                event = self._classify_activation()  # Use to classify dot, dash, or noise
                self.state = "GAP"
                self.gap_count = 1
                self.active_count = 0
        ...
        # 6.0 seconds triggers a word boundary.
            ...
            if is_active:
                ...
                if self.gap_count >= MIN_RELEASE_SAMPLES:
                    # Save gap count in case activation turns out to be noise
                    self.saved_gap_count = self.gap_count
                    self.state = "ACTIVE"
                    self.active_count = 1
                    self.gap_count = 0
    ...
    def _classify_activation(self):
        ...
        if self.active_count >= DASH_THRESHOLD_SAMPLES:
            ...
            # Noise detected: restore saved gap count so gap timer is not lost
            self.gap_count = self.saved_gap_count
...
if __name__ == "__main__":
    ...
    #filename = "test_how_are_you_morse_with_noise.csv"  # Change this to your actual file path
    #filename = "test_how_are_you_morse.csv"
    filename = "test_hi_morse_realistic.csv"
    #filename = "test_oh_io_morse_realistic.csv"
    

    detector, avg_mav = process_emg_file(filename)
    print("\n--- RESULTS ---")
